...
CHInstance.prototype.updateLayer = function () {
	...
	if (CesiumHeatmap.defaults.useEntitiesIfAvailable && this._cesium.entities) {
		...
		//groundprimitive
		const _geom=new Cesium.GeometryInstance({
			geometry:new Cesium.RectangleGeometry({
				rectangle:this._rectangle,
				vertexFormat: Cesium.MaterialAppearance.vertexFormat
			})
		});
		const _primitive=new Cesium.GroundPrimitive({
		...
		this._groundPrimitive = this._cesium.scene.primitives.add(_primitive);
...
CHInstance.prototype.remove = function(){
	if(this._layer){
		if (CHeatmap.defaults.useEntitiesIfAvailable && this._cesium.entities) {
			this._cesium.entities.remove(this._layer);
		}else{
			this._cesium.scene.imageryLayers.remove(this._layer);
		}
	...
	if(this._groundPrimitive){
		this._cesium.scene.primitives.remove(this._groundPrimitive);
