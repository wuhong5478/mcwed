    ...
    function CesiumWidget(container, options) {
        ...
        try {
            var scene = new Scene({
                ...
                //caoyang 阴影马赫带优化
                shadowMachBeltOptimization : options.shadowMachBeltOptimization,
                //caoyang 矢量AOI光照效果优化(镜面反射+昼夜感光)
                aoiLightEnhance : options.aoiLightEnhance || false,
            });
            this._scene = scene;
            ...
            if (!defined(globe)) 
            {
                //caoyang Globe栅格影像光照感应
                const _globeLightSensitivity=options.globeLightSensitivity || false ;
                //caoyang 近地视角栅格瓦片渲染效果优化
                const _nearGroundTileOptimization=options.nearGroundTileOptimization || false ;
                const _nearGroundTileLevel=options.nearGroundTileLevel || 19;
                globe = new Globe(ellipsoid, _globeLightSensitivity, _nearGroundTileOptimization, _nearGroundTileLevel);
            }
            if (globe !== false) 
            {
                scene.globe = globe;
